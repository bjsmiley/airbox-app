...
pub fn start(
    ...
    tokio::spawn(async move {
        ...
        let mut just_send_response = false;

        loop {
            debug!("transport_tx closed? {}", transport_tx.is_closed());
            tokio::select! {
                broadcast = app_rx.recv() => {
                    if let Some(event) = broadcast {
                        debug!("Sending {} to addr {}", event, addr);
                        match event {
                    ...
                    if let Some(result) = network {
                        match result {
                            Ok(frame) => {
                                ...
                                // this is hacky to avoid presence responses from self
                                if just_send_response {
                                    if let (DiscoveryEvent::PresenceResponse(_), addr) = frame {
                                        if local_addr == addr {
                                            just_send_request = false;
                                            continue;
                                        }
                                ...
                                debug!("Recieving {} from addr {}", frame.0, frame.1);
                        ...
                        debug!("None returned from udp discovery codec");
                        break;
                    }
                }
            }
        }
        debug!("Shutting down discovery event loop")
    });
